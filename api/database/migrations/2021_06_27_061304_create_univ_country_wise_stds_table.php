...
class CreateUnivCountryWiseStdsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('univ_country_wise_stds', function (Blueprint $table) {
            $table->id();
            $table->string('INSTITUTE_ID',15)->unsigned();
            $table->foreign('INSTITUTE_ID')->references('INSTITUTE_ID')->on('INSTITUTES');
            $table->string('COUNTRY_NAME',255);
            $table->integer('TOTAL_STUDENT',10);
            $table->integer('FEMALE_STUDENT',10);
    ...
    {
        Schema::dropIfExists('univ_country_wise_stds');
    }
}
