...
class TecStdSecondPageController extends Controller
{
    ...
    public function submitData(Request $request)
    {
        ...
        $instId = $data["instId"];
        $dipStudentData = $data["dipStudentData"];
        $stdBasicCrsdata = $data["stdBasicCrsdata"];
        $sscVocStd = $data["sscVocStd"];
        $hscVocStd = $data["hscVocStdData"];
        $hscBMStd = $data["hscBmStdData"];
        /*==================Parsing Table wise data from Array End=======*/

        /* saving Diploma_engineers */

        try {
            Diploma_engineers::where('institute_id', $instId)->delete();
            Diploma_engineers::insert($dipStudentData);
        } catch (\Exception $e) {
            array_push($err, $e);
        }

        /*saving data of Basic_crs_sammaries */
        try {
            Basic_crs_sammaries::where('id', $stdBasicCrsdata["id"])->update($stdBasicCrsdata);
