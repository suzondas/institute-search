...
use App\Models\Designation_mads;
...
class MadStdSeventhPageController extends Controller
{
    public function index($inst_id)
    {
        ...
        require_once app()->basePath('app') . '/Helpers/LookUpJsonMadSeventh.php';

        /*Teachers_staff_summary*/
        //$data->desigList= Designation_mads::orderBy('mad_sl','ASC')->get();
        ...
        if ($teachVocStafSum->isEmpty()) {
            ...
            foreach ($desigVocList as $desig) {
                ...
                $toInsertArrDV['designation_id'] = $desig['designation_id'];
                array_push($toAddedteachVocStafSum, $toInsertArrDV);
            }
            try {
        ...
        //$data->hdList= Lookup_hd_training::wherein('higher_degree_id', array(4,5,6,7,22))->orderBy('higher_degree_id')->get();
        $data->hdList= $hdListMad;
        $hdTeachSum = Higher_degree_teachers::where(['institute_id' => $inst_id])->get();
        if ($hdTeachSum->isEmpty()) {
            $toAddedhdTeachSum = [];
            foreach ($hdListMad as $hdVal) {
                $toInsertArrHdT = [];
