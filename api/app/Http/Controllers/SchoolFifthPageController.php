...
class SchoolFifthPageController extends Controller
{
    public function index($inst_id)
    {
        ...
        require_once app()->basePath('app') . '/Helpers/LookUpJsonSchoolFifth.php';

        /*Teachers_staff_summary*/
       // $data->desigList= Designation_schools::whereNotNull('sch_sl')->orderBy('sch_sl','ASC')->get();
        ...
        $teachVocStafSum = Technical_teachers_summaries::where(['institute_id' => $inst_id])->orderby('designation_id')->get();
        if ($teachVocStafSum->isEmpty()) {
            $toAddedteachVocStafSum = [];
            foreach ($desigVocList as $desig) {
                $toInsertArrDgv = [];
                $toInsertArrDgv['institute_id'] = $inst_id;
                $toInsertArrDgv['designation_id'] = $desig['designation_id'];
                array_push($toAddedteachVocStafSum, $toInsertArrDgv);
            }
            try {
